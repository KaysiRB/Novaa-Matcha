...
local function clearGunESP()
    for _, object in ipairs(gunEspObjects) do removeDrawing(object) end
    gunEspObjects = {}
end
...
local function updateESP()
    ...
    for cachedPlayer in pairs(roleCache) do
        ...
        if not exists then roleCache[cachedPlayer] = nil end
    end
            ...
            if character and root and humanoid and humanoid.Health > 0 then
                ...
                if not localRoot or distance <= MAX_DISTANCE then
                    ...
                    if headVisible and feetVisible then
                        ...
                        local text = ""
                        if SHOW_NAMES then text = player.Name end
                        if SHOW_ROLES then if text ~= "" then text = text .. " " end text = text .. "[" .. role .. "]" end
                        nameText.Text = text
                        ...
                        pcall(function() distanceText.FontSize = 12 end)
                        ...
                        espObjects[player] = {box = box, nameText = nameText, distText = distanceText, tracer = tracer}
                    end
                end
            end
        end
...
local function updateGunESP()
    ...
    if not ESP_ENABLED then return end
    if not SHOW_GUN_ESP then return end
            ...
            if position then
                ...
                if visible then
                    ...
                    if distance <= MAX_DISTANCE then
                        ...
                        pcall(function() text.FontSize = 14 end)
...
local function setESPEnabled(value)
    ...
    if not ESP_ENABLED then clearPlayerESP() clearGunESP() end
end
...
espToggle = mm2ESP:Toggle("Enabled", ESP_ENABLED, function(value) setESPEnabled(value) Lib:Notify("MM2 ESP", ESP_ENABLED and "Enabled" or "Disabled", 2, ESP_ENABLED and "success" or "warning") end)
espToggle:AddKeybind("O", "Toggle")

mm2ESP:Divider("Players")
mm2ESP:Toggle("Boxes", SHOW_BOXES, function(value) SHOW_BOXES = value end)
mm2ESP:Toggle("Names", SHOW_NAMES, function(value) SHOW_NAMES = value end)
mm2ESP:Toggle("Roles", SHOW_ROLES, function(value) SHOW_ROLES = value end)
mm2ESP:Toggle("Distance", SHOW_DISTANCE, function(value) SHOW_DISTANCE = value end)
mm2ESP:Toggle("Tracers", SHOW_TRACERS, function(value) SHOW_TRACERS = value end)
mm2ESP:Slider("Maximum distance", MAX_DISTANCE, 25, 50, 5000, " studs", function(value) MAX_DISTANCE = value end)

mm2ESP:Divider("Dropped gun")
mm2ESP:Toggle("Gun ESP", SHOW_GUN_ESP, function(value) SHOW_GUN_ESP = value if not value then clearGunESP() end end)
mm2ESP:Toggle("Gun distance", SHOW_GUN_DISTANCE, function(value) SHOW_GUN_DISTANCE = value end)
...
roleColors:Colorpicker("Murderer", COLORS.Murderer, function(color) COLORS.Murderer = color end)
roleColors:Colorpicker("Sheriff", COLORS.Sheriff, function(color) COLORS.Sheriff = color end)
roleColors:Colorpicker("Innocent", COLORS.Innocent, function(color) COLORS.Innocent = color end)
roleColors:Colorpicker("Dropped gun", COLORS.Gun, function(color) COLORS.Gun = color end)
...
utilities:Button("Refresh ESP", function() updateESP() updateGunESP() Lib:Notify("Novaa", "ESP refreshed", 1, "success") end)
utilities:Button("Unload", function() Lib:Dialog({title = "Unload Novaa?", text = "This will remove all drawings and close the UI.", confirm = "Unload", onConfirm = function() _G.MM2ESP_Running = false if renderConnection then renderConnection:Disconnect() renderConnection = nil end if _G.MM2ESP_Cleanup then _G.MM2ESP_Cleanup() end Lib:Destroy() end, }) end):SetRisk()
...
renderConnection = RunService.RenderStepped:Connect(function(dt)
    if not _G.MM2ESP_Running then
        ...
        if _G.MM2ESP_Cleanup then _G.MM2ESP_Cleanup() end return
    end

    espTick += dt
    if espTick >= 0.15 then -- ~6 updates/seconde
        espTick = 0
