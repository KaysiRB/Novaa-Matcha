...
local function loadInsUI()
    if type(INSui) == "table" then
        return INSui
    end
    local ok, result = pcall(function()
        return loadstring(game:HttpGet(BASE_URL .. "Utils/Scripts/INS-UI.lua"))()
    end)
...
function Game:buildUI()
    local Lib = self.env and self.env.INSui or _G.INSui or loadInsUI()
    self.Lib = Lib
    if not Lib then
        warn("MM2: INS-UI unavailable")
        return
    end

    self.win = Lib:CreateWindow({
        title = "Novaa - v1.0.0",
        subtitle = "auto",
    ...
    self.win:AddSettingsTab("gear")
    Lib:Notify("Novaa", "Press LeftControl to toggle the menu", 4, "info")
    Lib:Category("Games")

    local gameInfoTab = self.win:Tab("Game", "gamepad")
    ...
    local espTab = self.win:Tab("ESP", "users")
    local mm2ESP = espTab:Section("MM2 ESP", "Full", "Murderer, Sheriff, Innocent and dropped gun")
    local espToggle = mm2ESP:Toggle("Enabled", ESP_ENABLED, function(value)
        setESPEnabled(value)
    ...
    local gameSettingsTab = self.win:Tab("Game Settings", "cog")
    ...
    self.win:autoloadConfig("MM2")
    Lib:Notify("Loaded", "Novaa script is ready!", 3, "success")
end

function Game:Start()
    self.running = true
    if type(self.env.RegisterModule) == "function" then
        self.env:RegisterModule("MM2", self)
    end
    self:buildUI()

    self.renderConnection = RunService.RenderStepped:Connect(function(dt)
        if not self.running or self.env.IsUnloadRequested() then
            self:Unload()
            return
        end
        self.espTick = self.espTick + dt
        if self.espTick >= 0.15 then
            self.espTick = 0
            updateESP()
            updateGunESP()
        end
...
function Game:Unload()
    ...
    if self.env and type(self.env.UnregisterModule) == "function" then
        self.env:UnregisterModule("MM2")
    end
end

return function(env)
    local self = setmetatable({env = env}, {__index = Game})
    self:Start()
    return self
end
