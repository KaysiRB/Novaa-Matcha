--[[
    Novaa - Driving Empire
    Delivery Driver auto-farm with live stats dashboard.
]]
...
local function fmtMoney(v) return "$" .. comma(v) end
...
local function readCash()
	...
	local ok, v = pcall(function() return tonumber(c.Value) end)
...
local function pruneSamples(t, now, window)
	...
	while #t > 2 and t[1].t < cutoff do table.remove(t, 1) end
end

local function addSample(now, bal)
	stats.samples[#stats.samples + 1] = { t = now, cash = bal }
...
local function avgPayout()
	if stats.payouts <= 0 then return 0 end
	return stats.posGain / stats.payouts
end

local function computeGain()
	local gain = stats.totalGain
	if stats.currBal and stats.startBal then gain = stats.currBal - stats.startBal end
	return gain
end
...
local function updateCash(force)
	local now = os.clock()
	if not force and now - stats.lastTick < 1 then return end
	stats.lastTick = now
	...
	if stats.lastBal == nil then stats.lastBal = bal end
	...
	if delta >= CONFIG.MIN_GAIN then
		...
		if delta > stats.largestGain then stats.largestGain = delta end
	end
...
local function resetStats()
	...
	stats.samples = {}
	if bal then addSample(now, bal) end
end
...
local function horizontalApproach(pos, target)
	...
	return Vector3.new(
		target.X + (dx / len) * CONFIG.APPROACH,
		target.Y + heightOffset(),
		target.Z + (dz / len) * CONFIG.APPROACH
	)
end
...
local function holdPos(root, pos, dur)
	...
	while os.clock() < deadline do
		if _G.DELIVERY_FARM_GEN ~= SCRIPT_GEN then return end
		root.Position = pos
		root.Velocity = z
		task.wait()
	end
...
if not Lib then
	Lib = {
		CreateWindow = function()
			return {
				Tab = function() return { Section = function() return {
					Label = function() end, Info = function() end,
					Divider = function() end, Button = function() end,
					Toggle = function() return { AddKeybind = function() end, Tooltip = function() end } end,
				} end } end,
				AddSettingsTab = function() end,
				autoloadConfig = function() end,
				Notify = function() end, Category = function() end,
				ApplyThemePreset = function() end, Dialog = function() end,
...
local function toggleFarm()
	if os.clock() - state.lastToggle < 0.3 then return end
...
local farmToggle = afSec:Toggle("Farm Deliveries", state.enabled, function(v)
	if v and not state.enabled then toggleFarm()
	elseif not v and state.enabled then toggleFarm() end
end)
farmToggle:AddKeybind(CONFIG.TOGGLE_KEY, "Toggle")
farmToggle:Tooltip("Press " .. CONFIG.TOGGLE_KEY .. " to toggle farming")

afSec:Toggle("Privacy Mode", state.privacy, function(v)
	state.privacy = v == true
end)

afSec:Info("Enable noclip")
...
-- Backup: manually refresh label handles in the main loop
local function updateStats()
	if statLabels.status then statLabels.status:Set("Status: " .. (state.enabled and "Running" or "Paused")) end
	if statLabels.runtime then statLabels.runtime:Set("Runtime: " .. fmtDur(os.clock() - stats.startedAt)) end
	if statLabels.active then statLabels.active:Set("Active: " .. fmtDur(activeTime())) end
	if statLabels.cashMade then statLabels.cashMade:Set("Cash Made: " .. fmtMoney(computeGain())) end
	if statLabels.balance then statLabels.balance:Set("Balance: " .. fmtMoney(stats.currBal or stats.startBal or 0)) end
	if statLabels.lastGain then statLabels.lastGain:Set("Last Gain: " .. fmtMoney(stats.lastGain)) end
	if statLabels.payouts then statLabels.payouts:Set("Payouts: " .. comma(stats.payouts)) end
	if statLabels.avgPayout then statLabels.avgPayout:Set("Avg/Payout: " .. fmtMoney(avgPayout())) end
	if statLabels.estHr then statLabels.estHr:Set("EST/hr: " .. fmtMoney(estPH(60))) end
	if statLabels.pickups then statLabels.pickups:Set("Pickups: " .. comma(stats.pickups)) end
	if statLabels.dropoffs then statLabels.dropoffs:Set("Dropoffs: " .. comma(stats.dropoffs)) end
	if statLabels.fails then statLabels.fails:Set("Fails: " .. comma(stats.fails)) end
	if statLabels.largestGain then statLabels.largestGain:Set("Largest Gain: " .. fmtMoney(stats.largestGain)) end
	if statLabels.targetVisits then statLabels.targetVisits:Set("Target Visits: " .. comma(stats.targetVisits)) end
end

createStatsUI()

--// UNIVERSAL
win:Category("Universal")
local visTab = win:Tab("Visuals", "eye")
visTab:Section("Universal Visuals", "Full", "Features shared between supported games"):Info("Coming soon")

local plTab = win:Tab("Player", "user")
plTab:Section("Movement", "Full", "Universal player settings"):Info("Coming soon")

local miscTab = win:Tab("Misc", "three-dots-horizontal")
local utils = miscTab:Section("Utilities", "Full")
utils:Button("Unload", function()
	win:Dialog({
		title = "Unload Novaa?",
		text = "This will destroy the UI and stop all scripts.",
		confirm = "Unload",
		onConfirm = function()
